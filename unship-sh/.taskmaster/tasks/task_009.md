...
**Description:** Set up the CLI with Clap, implement project root discovery by walking upward to find .unship/, and establish global flags and subcommand structure.

**Details:**

Create the CLI foundation:

1. Update `crates/unship-cli/Cargo.toml`:
```toml
[dependencies]
clap = { version = "4.5", features = ["derive"] }
unship-core = { path = "../unship-core" }
colored = "2.0"
```

2. Create `crates/unship-cli/src/cli.rs`:
```rust
use clap::{Parser, Subcommand};

#[derive(Parser)]
#[command(name = "unship", version, about = "Local-first project companion")]
pub struct Cli {
    #[command(subcommand)]
...
1. Run `cargo run -p unship -- --help` - verify help output shows all commands.
2. Run `cargo run -p unship -- --version` - verify version output.
3. Test project root discovery finds .unship/ in current directory.
4. Test project root discovery finds .unship/ in parent directory.
