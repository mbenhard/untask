...
1. Create `crates/unship-core/src/docs.rs`:
```rust
pub struct Doc {
    pub path: PathBuf,
    pub basename: String,
...
2. Doc discovery:
   ...
   - Include `.unship/docs/**/*.md` by default
   - Handle duplicate basenames: return Ambiguous error with all matching paths

3. Create `crates/unship-core/src/search.rs`:
```rust
...
Create `crates/unship-core/tests/docs_test.rs` and `search_test.rs`:
...
6. Test configured doc globs outside `.unship/docs` are discovered.
