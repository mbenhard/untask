...
`list_docs`, `read_doc`, and `save_doc` must honor configured `config.docs` globs through the shared docs discovery layer rather than assuming `.unship/docs/` only.

3. Persist data to `~/Library/Application Support/Unship/`:
