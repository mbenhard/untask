...
1. Create `crates/unship-core/src/store.rs`:
```rust
...
3. Write behavior:
   - Acquire `.unship/.lock` before any mutation
...
4. Read behavior:
   ...
   - Parse all .md files in `.unship/tasks/`
   - Classify as managed/unindexed but don't fix

**Test Strategy:**

Create `crates/unship-core/tests/store_test.rs`:
