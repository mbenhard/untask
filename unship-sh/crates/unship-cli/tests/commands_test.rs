...
fn configure_docs(dir: &Path, patterns: &[&str]) {
    let docs = patterns
        ...
        .collect::<Vec<_>>()
        .join("\n");
    fs::write(dir.join(".unship/config.yml"), format!("docs:\n{docs}\n")).unwrap();
}
...
fn init_creates_project() {
    ...
    assert!(tmp.path().join(".unship/tasks").is_dir());
}
...
fn bare_command_prints_help_and_guidance_inside_project() {
    ...
    assert!(stdout.contains("Use a CLI subcommand or `unship open` to launch the desktop app."));
}
...
fn bare_command_json_returns_error_without_help_text() {
    ...
    assert_eq!(
        parsed["error"],
        "Use a CLI subcommand or `unship open` to launch the desktop app."
    );
}
...
fn add_creates_task_file() {
    ...
    // Verify file exists
    let entries: Vec<_> = std::fs::read_dir(tmp.path().join(".unship/tasks"))
        .unwrap()
        .collect();
...
fn edit_supports_editor_commands_with_arguments() {
    ...
    let (_, stderr, ok) = run_in_with(
        ...
        &[
            ...
            ("UNSHIP_EDITOR_LOG", log_path.to_str().unwrap()),
        ],
    );
    ...
    assert!(recorded.contains(".unship/tasks/001-editable-task.md"));
}
...
fn delete_without_force_can_be_cancelled() {
    ...
    let entries: Vec<_> = fs::read_dir(tmp.path().join(".unship/tasks"))
        .unwrap()
        .filter_map(|entry| entry.ok())
...
fn docs_list_shows_all_discovered_docs() {
    let tmp = TempDir::new().unwrap();
    init_project(tmp.path());
    configure_docs(tmp.path(), &[".unship/docs/**/*.md", "docs/**/*.md"]);
    write_doc(tmp.path(), ".unship/docs/guide.md", "# Guide\n");
    write_doc(tmp.path(), "docs/plan.md", "# Plan\n");

    let (stdout, _, ok) = run_in(tmp.path(), &["docs"]);
    assert!(ok);
    assert!(stdout.contains(".unship/docs/guide.md"));
...
fn docs_show_with_ambiguous_name_returns_helpful_error() {
    ...
    init_project(tmp.path());
    configure_docs(tmp.path(), &[".unship/docs/**/*.md", "docs/**/*.md"]);
    write_doc(tmp.path(), ".unship/docs/notes.md", "root notes");
    ...
    assert!(stderr.contains(".unship/docs/notes.md"));
...
fn docs_list_discovers_docs_folder_without_config() {
    let tmp = TempDir::new().unwrap();
    fs::create_dir(tmp.path().join(".unship")).unwrap();
...
fn repair_check_reports_issues_without_modifying_files() {
    ...
    let before = fs::read_to_string(tmp.path().join(".unship/tasks/loose-note.md")).unwrap();
    ...
    assert!(stdout.contains("Run 'unship repair --write' to fix these issues."));

    let after = fs::read_to_string(tmp.path().join(".unship/tasks/loose-note.md")).unwrap();
...
fn repair_write_fixes_issues_and_json_output() {
    ...
    assert!(tmp.path().join(".unship/tasks/001-loose-note.md").exists());
...
fn skill_install_copies_bundled_skill_when_supported_path_exists() {
    ...
    assert!(commands_dir.join("unship.md").exists());
    assert!(commands_dir.join("unship-finish.md").exists());
    assert!(commands_dir.join("unship-docs.md").exists());
    assert!(commands_dir.join("unship-batch.md").exists());

    let content = fs::read_to_string(commands_dir.join("unship.md")).unwrap();
    assert!(content.contains("unship next --json"));
...
fn open_fails_gracefully_when_app_is_unavailable() {
    ...
    assert!(!ok);
    assert!(stderr.contains("Could not open the Unship desktop app"));
}
