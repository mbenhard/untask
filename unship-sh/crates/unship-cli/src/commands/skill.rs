...
const SKILL_UNSHIP: &str = include_str!("../../skill/unship.md");
const SKILL_FINISH: &str = include_str!("../../skill/unship-finish.md");
const SKILL_DOCS: &str = include_str!("../../skill/unship-docs.md");
const SKILL_BATCH: &str = include_str!("../../skill/unship-batch.md");
...
fn install_claude_code(home: &Path, json: bool) -> Result<()> {
    ...
    if !claude_dir.is_dir() {
        return emit_install_result(
            ...
            &[],
            Some(
                "No supported agent config found. Create ~/.claude/commands/ and run `unship skill install` again.",
            ),
        );
    }
...
fn install_codex(json: bool) -> Result<()> {
    ...
    if path.exists() {
        ...
        if !existing.contains("# unship") {
            let appended = format!("{existing}\n\n---\n\n{content}");
            std::fs::write(&path, appended)?;
        }
        // Already contains unship skills — skip
    } else {
...
fn resolve_home_dir() -> Result<PathBuf> {
    std::env::var_os("HOME")
        ...
        .ok_or_else(|| {
            UnshipError::Io(std::io::Error::new(
