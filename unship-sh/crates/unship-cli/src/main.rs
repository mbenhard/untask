...
const NO_COMMAND_MESSAGE: &str = "Use a CLI subcommand or `unship open` to launch the desktop app.";
...
fn run(cli: &Cli, fmt: &Formatter) -> unship_core::error::Result<()> {
    match &cli.command {
        None => unreachable!(),
        Some(Commands::Init) => {
            let root = std::env::current_dir()?;
            unship_core::init::init(&root, None)?;
            println!("Initialized unship project in {}", root.display());
            Ok(())
        }
        Some(cmd) => {
            // All other commands require an initialized project
            let cwd = std::env::current_dir()?;
            let root = unship_core::project::find_project_root(&cwd)?;
