...
pub enum Commands {
    /// Initialize a new unship project
    Init,
...
pub enum SkillCommands {
    /// Install the unship skill for an AI agent
    Install {
...
mod tests {
    ...
    fn parses_global_flags() {
        let cli = Cli::try_parse_from(["unship", "--json", "--no-color", "list"]).unwrap();
    ...
    fn parses_nested_subcommands() {
        let docs = Cli::try_parse_from(["unship", "docs", "show", "plan"]).unwrap();
        assert!(matches!(
            docs.command,
            Some(Commands::Docs {
        ...
        let skill = Cli::try_parse_from(["unship", "skill", "install"]).unwrap();
        assert!(matches!(
            skill.command,
            Some(Commands::Skill {
    ...
    fn parses_docs_path_subcommands() {
        ...
        let remove =
            Cli::try_parse_from(["unship", "docs", "remove-path", "docs/**/*.md"]).unwrap();
        assert!(matches!(
            remove.command,
    ...
    fn rejects_conflicting_repair_flags() {
        let err = Cli::try_parse_from(["unship", "repair", "--check", "--write"]).unwrap_err();
