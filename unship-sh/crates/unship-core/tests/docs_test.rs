use unship_core::docs::{DocNode, DocNodeKind, DocsStore, infer_writable_doc_root};
...
fn write_config(tmp: &tempfile::TempDir, content: &str) {
    std::fs::write(tmp.path().join(".unship/config.yml"), content).unwrap();
}
...
fn list_discovers_nested_docs() {
    let tmp = setup();
    write_doc(&tmp, ".unship/docs/deep/nested/doc.md", "nested content");
...
fn list_handles_missing_docs_directory() {
    let tmp = setup();
    // Don't create .unship/docs/ — it shouldn't exist yet
    let store = DocsStore::new(tmp.path().to_path_buf());
    let docs = store.list().unwrap();
...
fn get_by_unique_basename() {
    let tmp = setup();
    write_doc(&tmp, ".unship/docs/setup.md", "# Setup\nInstructions here.");
...
fn get_ambiguous_basename_returns_error() {
    let tmp = setup();
    write_doc(&tmp, ".unship/docs/notes.md", "notes v1");
    write_doc(&tmp, ".unship/docs/sub/notes.md", "notes v2");
    ...
    assert!(matches!(
        err,
        unship_core::error::UnshipError::Ambiguous(ref name, _) if name == "notes.md"
    ));

    // Error message should list both paths
    let msg = err.to_string();
    assert!(msg.contains(".unship/docs/"));
}
...
fn get_not_found_returns_error() {
    ...
    assert!(matches!(
        err,
        unship_core::error::UnshipError::DocNotFound(_)
    ));
}
...
fn get_by_relative_path_disambiguates() {
    let tmp = setup();
    write_doc(&tmp, ".unship/docs/notes.md", "root notes");
    write_doc(&tmp, ".unship/docs/sub/notes.md", "sub notes");

    let store = DocsStore::new(tmp.path().to_path_buf());
    let doc = store.get(".unship/docs/sub/notes.md").unwrap();
...
fn get_is_case_sensitive() {
    let tmp = setup();
    write_doc(&tmp, ".unship/docs/README.md", "readme content");
...
fn infer_writable_doc_root_only_accepts_literal_directory_prefixes() {
    ...
    assert_eq!(
        infer_writable_doc_root(".unship/docs/*.md").unwrap(),
        std::path::PathBuf::from(".unship/docs")
    );
