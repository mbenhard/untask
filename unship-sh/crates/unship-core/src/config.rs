...
fn default_docs() -> Vec<String> {
    vec![".unship/docs/**/*.md".into(), "docs/**/*.md".into()]
}
...
impl Config {
    ...
    /// Load config from `.unship/config.yml`.
    ...
    /// Load config from `.unship/config.yml`, surfacing invalid config as an error.
    ...
    /// Save config to `.unship/config.yml`.
    pub fn save(&self, project_root: &Path) -> Result<()> {
        let config_path = project_root.join(".unship/config.yml");
        let content = serde_yaml::to_string(self)
            .map_err(|e| UnshipError::InvalidConfig(format!("failed to serialize config: {e}")))?;
    ...
    fn find_column_index(&self, name: &str) -> Result<usize> {
        ...
        self.columns
            ...
            .ok_or_else(|| UnshipError::InvalidConfig(format!("column not found: {name}")))
    }
