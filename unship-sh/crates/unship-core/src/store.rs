...
impl TaskStore {
    ...
    fn tasks_dir(&self) -> PathBuf {
        self.project_root.join(".unship/tasks")
    }
    ...
    fn load_task_by_id(&self, id: u32) -> Result<Task> {
        let path = self
            ...
            .ok_or_else(|| UnshipError::TaskNotFound(id.to_string()))?;
    ...
    pub fn get_by_ref(&self, reference: &str) -> Result<Task> {
        ...
        match matches.len() {
            0 => Err(UnshipError::TaskNotFound(reference.to_string())),
    ...
    pub fn delete_attachment(&self, id: u32, filename: &str) -> Result<Task> {
        ...
        let removed = remove_attachment_entry(&mut task.attachments, filename)
            .ok_or_else(|| UnshipError::TaskNotFound(format!("attachment {filename}")))?;
    ...
    pub fn attachment_path(&self, id: u32, filename: &str) -> Result<PathBuf> {
        ...
        let path =
            crate::attachments::attachment_path(&self.project_root, task_id, &attachment.filename)?;
        if !path.is_file() {
            return Err(UnshipError::TaskNotFound(format!("attachment {filename}")));
        }
        Ok(path)
    }
    ...
    pub fn read_attachment_text(&self, id: u32, filename: &str) -> Result<AttachmentTextPreview> {
        ...
        let extension = Path::new(&attachment.filename)
            ...
            .ok_or_else(|| {
                UnshipError::InvalidConfig(format!(
        ...
        let path =
            crate::attachments::attachment_path(&self.project_root, task_id, &attachment.filename)?;
        if !path.is_file() {
            return Err(UnshipError::TaskNotFound(format!(
                "attachment {}",
                attachment.filename
            )));
        }
    ...
    fn normalize_status(&self, raw: &str) -> Result<String> {
        self.config
            ...
            .ok_or_else(|| UnshipError::InvalidConfig(format!("unknown status: {raw}")))
    }
    ...
    loop {
        match String::from_utf8(bytes.clone()) {
            ...
            Err(err) => {
                ...
                if valid_up_to == 0 {
                    return Err(UnshipError::InvalidConfig(
