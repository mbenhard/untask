...
```
AI works -> unship done <id> -> task disappears into "done" column -> hope it's fine
```

Additionally, the `/unship` skill is monolithic — it loads all instructions regardless of what the agent is doing, wasting context budget. And there's no structured way for the AI to batch-process multiple tasks efficiently.
...
New section in `.unship/config.yml`:
...
Note: the `review` column already exists in the default kanban preset with aliases `["reviewing", "in review"]`, so `unship status <id> review` works today.

---

## 4. Skill Split

The monolithic `/unship` skill splits into four focused skills:

### `unship` (session start)

Fires at start of a work session.

- Run `unship next --json` to see open tasks
- Pick a task and run `unship status <id> in-progress`
- Use `unship search <query>` to find relevant tasks
- Use `unship list --status <status> --json` to see tasks in a specific column
...
- Read `auto_done` from `.unship/config.yml`
  - If `false` or missing: `unship status <id> review`
  - If `true`: `unship done <id>`
- Run `unship repair --check` to verify project integrity

### `unship-docs` (working with docs)

Fires when AI needs to work with project documentation.

- How to use `.unship/docs/`
- Create/edit docs and PRD conventions
- `unship docs` and `unship docs show <name>` commands

### `unship-batch` (parallel task processing)

Fires when the AI wants to process multiple tasks or the user asks for batch work.

#### Flow

1. **Scan:** `unship list --status todo --json` to get actionable tasks from a specific column
...
4. **Execute:** After human confirms, dispatch work:
   ...
   - Each task follows `unship-finish` conventions (summary, confidence, etc.)
...
- **Skill not firing:** Skills are convention-based, not enforced. If an agent runs `unship done` without loading `unship-finish`, the task goes to done without a summary. The review view handles this gracefully.
...
- **Skill installation:** Current `unship skill install` writes a single file to `~/.claude/commands/unship.md`. With the split, it writes multiple files. Add `--provider` flag to target different agents:
...
| Rust backend | Update `unship skill install` for multiple files + `--provider` flag |
...
| Skill — unship.md | Trim to session-start only, add `--status` filter |
| Skill — unship-finish.md | New: task completion conventions |
| Skill — unship-docs.md | New: doc workflow instructions |
| Skill — unship-batch.md | New: parallel task processing |
