...
- Implemented validated attachment filenames, safe path resolution, transactional store-level add/remove flows, and text preview support in `unship-core`.
...
- `cargo test -p unship-core -- --nocapture`
...
- The attachment asset protocol scope is now restricted to `**/.unship/attachments/**`; this should work in dev and packaged builds, but runtime preview behavior should still be smoke-tested once in the real app because Tauri glob matching is environment-sensitive.
