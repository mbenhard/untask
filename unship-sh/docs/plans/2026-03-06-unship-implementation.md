# Unship v1.0 Implementation Plan

**Goal:** Build Unship as a local-first Rust workspace with a shared core, a CLI binary, and a macOS desktop app that all read and write the same `.unship/` project data.

**Design Doc:** `docs/plans/2026-03-06-unship-design.md`
...
3. Every mutating filesystem operation acquires `.unship/.lock`.
...
3. Explicit desktop launch via `unship open`.

### Phase 3: Desktop App

1. Tauri backend commands layered on `unship-core`.
...
1. CLI and desktop both operate on the same `.unship/` data.
