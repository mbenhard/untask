...
- Taskmaster 8 (`Git Summary and Next Command Implementation`) is implemented in [crates/unship-core/src/git.rs](/Users/marcusbenhard/Development/untitled/unship-sh/crates/unship-core/src/git.rs), [crates/unship-core/src/next.rs](/Users/marcusbenhard/Development/untitled/unship-sh/crates/unship-core/src/next.rs), and [crates/unship-core/tests/next_test.rs](/Users/marcusbenhard/Development/untitled/unship-sh/crates/unship-core/tests/next_test.rs).
- The repo-level implementation plan [docs/plans/2026-03-06-unship-implementation.md](/Users/marcusbenhard/Development/untitled/unship-sh/docs/plans/2026-03-06-unship-implementation.md) labels a different CLI scope as task 8, so this audit uses Taskmaster’s generated task file as the canonical source for scope and numbering.

## Findings (by severity)

- P1: `generate_next` classified tasks by the raw `status` string instead of the configured canonical status map, so valid done aliases like `finished` or `closed` were incorrectly shown as open work and omitted from recently completed output. Fixed in [crates/unship-core/src/next.rs](/Users/marcusbenhard/Development/untitled/unship-sh/crates/unship-core/src/next.rs) with regression coverage in [crates/unship-core/tests/next_test.rs](/Users/marcusbenhard/Development/untitled/unship-sh/crates/unship-core/tests/next_test.rs).
...
## Test Delta
- Before:
  - `cargo test -p unship-core --test next_test` -> passed (12 passed)
  - `cargo test -p unship-core` -> passed (109 passed)
- After:
  - `cargo test -p unship-core --test next_test` -> passed (13 passed)
...
- `cargo fmt --all`
- `cargo test -p unship-core --test next_test`
