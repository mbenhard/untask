...
- Preserve the current dense monochrome Unship design language.
...
- Must follow `docs/unship-design-language.md`: monochrome, dense spacing, borders over fills, restrained motion, Geist + Geist Mono.
...
Top-level tree nodes are writable or read-only doc roots inferred from config, for example `docs/` and `.unship/docs/`. Under each root, show folders and docs by filesystem path. Never merge folders across different roots even if names match.
