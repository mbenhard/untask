...
Unship is built around kanban/list for complex, multi-task projects. But many projects are simple — a WordPress plugin, an internal tool, a quick client app. These don't need task breakdown. You write a PRD, hand it to an AI agent, and it gets built in one shot.

There's no place in unship for this workflow today.
...
A PRD is a markdown file in `.unship/docs/` with `type: prd` in frontmatter:
...
2. **Filesystem** — Drop a `.md` file into `.unship/docs/` with `type: prd` frontmatter, or edit in any text editor.

**AI side:**
3. **CLI** — `unship doc create --type prd "My Project"` writes the file. Or the agent writes the file directly.
...
**Agent-specific instructions** (skills, rules files) teach each tool how to interact with unship:
- Claude Code: unship skill
- Cursor: `.cursor/rules`
...
- Check for PRDs: `unship doc list --type prd`
...
- When generating a PRD: write to `.unship/docs/` with `type: prd` frontmatter
...
Recommend the core helper since the CLI will also want `unship doc list --type prd` to optionally show linked task counts.

### PRD location is frontmatter-driven, not folder-driven

PRDs can live in any discovered docs root (`.unship/docs/`, `docs/`, or any custom glob in config). The `type: prd` frontmatter is the sole identifier — location doesn't matter. A PRD in `docs/specs/my-project.md` is just as valid as one in `.unship/docs/my-project.md`.
