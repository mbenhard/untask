...
**Tech Stack:** Rust (unship-core, unship-cli), Tauri 2 IPC commands, Svelte 5 (desktop app)

**Key decision:** The `prd` field on tasks stores the **relative path** (e.g., `.unship/docs/my-project.md`), not just the basename. This avoids ambiguity when PRDs in different folders share a name.
...
**Files:**
- Modify: `crates/unship-core/src/task.rs:8-61`
- Modify: `crates/unship-core/src/store.rs` (add method, TaskUpdate)
- Test: `crates/unship-core/tests/store_test.rs`

**Step 1: Write the failing tests**

Add to the bottom of `crates/unship-core/tests/store_test.rs`:
...
fn add_task_with_prd_sets_field() {
    ...
    let task = store.add("Task from PRD", None, Some(".unship/docs/spec.md")).unwrap();
...
Run: `cargo test -p unship-core --test store_test add_task_preserves_prd`
...
In `crates/unship-core/src/task.rs`, add to `Task` struct (after `tags`):
...
In `crates/unship-core/src/store.rs`, add `prd` to `TaskUpdate`:
...
- `crates/unship-core/src/store.rs` — `add(title, status)` → `add(title, status, None)` (in tests or internal calls if any)
- `crates/unship-cli/src/commands/add.rs` — update the call
- `apps/desktop/src-tauri/src/commands.rs` — update `add_task` command
- `crates/unship-core/tests/store_test.rs` — all existing `store.add(...)` calls

**Step 5: Run tests to verify they pass**

Run: `cargo test -p unship-core`
Expected: All tests pass

**Step 6: Commit**

```bash
git add crates/unship-core/src/task.rs crates/unship-core/src/store.rs crates/unship-core/tests/store_test.rs crates/unship-cli/src/commands/add.rs apps/desktop/src-tauri/src/commands.rs
...
**Files:**
- Modify: `crates/unship-core/src/docs.rs:1-20`
- Test: `crates/unship-core/tests/docs_test.rs`

**Step 1: Write the failing tests**

Add to the bottom of `crates/unship-core/tests/docs_test.rs`:

```rust
#[test]
fn list_parses_doc_type_from_frontmatter() {
    let tmp = setup();
    write_doc(&tmp, ".unship/docs/spec.md", "---\ntype: prd\n---\n# My PRD\nBuild this.");
    write_doc(&tmp, ".unship/docs/notes.md", "# Just notes\nNo frontmatter.");
    ...
    assert_eq!(spec.doc_type, unship_core::docs::DocType::Prd);
    assert_eq!(notes.doc_type, unship_core::docs::DocType::Doc);
}

#[test]
fn list_parses_doc_type_defaults_to_doc() {
    let tmp = setup();
    write_doc(&tmp, ".unship/docs/guide.md", "---\ntitle: Guide\n---\n# Guide");

    let store = DocsStore::new(tmp.path().to_path_buf());
    let docs = store.list().unwrap();

    assert_eq!(docs[0].doc_type, unship_core::docs::DocType::Doc);
}

#[test]
fn list_parses_explicit_doc_type() {
    let tmp = setup();
    write_doc(&tmp, ".unship/docs/notes.md", "---\ntype: doc\n---\n# Notes");
...
```bash
git add crates/unship-core/src/docs.rs crates/unship-core/tests/docs_test.rs
...
**Files:**
- Modify: `crates/unship-core/src/docs.rs` (DocNode struct, tree building)
- Test: `crates/unship-core/tests/docs_test.rs`

**Step 1: Write the failing test**

Add to `crates/unship-core/tests/docs_test.rs`:
...
fn list_tree_includes_doc_type_on_doc_nodes() {
    let tmp = setup();
    write_doc(&tmp, ".unship/docs/spec.md", "---\ntype: prd\n---\n# Spec");
    write_doc(&tmp, ".unship/docs/notes.md", "# Notes");

    let store = DocsStore::new(tmp.path().to_path_buf());
    let tree = store.list_tree().unwrap();

    let spec_node = find_node(&tree, ".unship/docs/spec.md").unwrap();
    let notes_node = find_node(&tree, ".unship/docs/notes.md").unwrap();

    assert_eq!(spec_node.doc_type, Some(unship_core::docs::DocType::Prd));
    assert_eq!(notes_node.doc_type, Some(unship_core::docs::DocType::Doc));
}
```

**Step 2: Run test to verify it fails**

Run: `cargo test -p unship-core --test docs_test list_tree_includes_doc_type`
Expected: FAIL — `DocNode` has no `doc_type` field

**Step 3: Add `doc_type` to DocNode and wire through tree building**

Add to `DocNode` struct in `crates/unship-core/src/docs.rs`:
...
**Files:**
- Modify: `crates/unship-core/src/store.rs`
- Test: `crates/unship-core/tests/store_test.rs`

**Step 1: Write the failing tests**

Add to `crates/unship-core/tests/store_test.rs`:
...
fn count_by_prd_returns_done_and_total() {
    ...
    let t1 = store.add("Task 1", None, Some(".unship/docs/my-project.md")).unwrap();
    let t2 = store.add("Task 2", None, Some(".unship/docs/my-project.md")).unwrap();
    let t3 = store.add("Task 3", None, Some(".unship/docs/my-project.md")).unwrap();
...
Run: `cargo test -p unship-core --test store_test count_by_prd`
Expected: FAIL — `TaskStore` has no method `count_by_prd`

**Step 3: Implement `count_by_prd`**

Add to `TaskStore` in `crates/unship-core/src/store.rs`:
...
```bash
git add crates/unship-core/src/store.rs crates/unship-core/tests/store_test.rs
...
**Files:**
- Modify: `crates/unship-cli/src/cli.rs:136-160` (DocsCommands::List, Commands::Add)
- Modify: `crates/unship-cli/src/commands/docs.rs:8-36` (list function)
- Modify: `crates/unship-cli/src/commands/add.rs`
- Modify: `crates/unship-cli/src/main.rs:115-127` (dispatch)

**Step 1: Add `--type` flag to DocsCommands::List**

In `crates/unship-cli/src/cli.rs`, update `DocsCommands::List`:
...
In `crates/unship-cli/src/cli.rs`, update `Commands::Add`:
...
In `crates/unship-cli/src/commands/docs.rs`, update `list` signature:
...
pub fn list(root: &Path, doc_type: Option<&str>, json: bool) -> Result<()> {
    ...
    if let Some(type_filter) = doc_type {
        let filter_type = match type_filter {
            "prd" => unship_core::docs::DocType::Prd,
            "doc" => unship_core::docs::DocType::Doc,
            other => return Err(UnshipError::InvalidConfig(format!("unknown doc type: {other}"))),
        };
...
In `crates/unship-cli/src/main.rs`, update lines 115-127. The current dispatch:
...
Run: `cargo test -p unship-cli`
Expected: All tests pass

**Step 6: Test manually**

Run: `cargo run -p unship -- docs list --type prd`
Expected: Lists only PRD docs (or "No docs found." if none)

Run: `cargo run -p unship -- add "My task" --prd .unship/docs/spec.md`
Expected: Creates a task with `prd` field

**Step 7: Commit**

```bash
git add crates/unship-cli/src/cli.rs crates/unship-cli/src/commands/docs.rs crates/unship-cli/src/commands/add.rs crates/unship-cli/src/main.rs
...
`DocNode` already gets `doc_type` from core (Task 3) — `DocNode` is serialized directly from `unship_core::docs::DocNode`, so the new field appears automatically in `list_docs_tree`. No changes needed.
...
2. Create tasks with `prd` linkage: `cargo run -p unship -- add "Build feature" --prd .unship/docs/spec.md`
3. View the PRD in the app — linked task count appears (e.g., "1 tasks · 0 done")
4. Mark task done: `cargo run -p unship -- done 1`
5. Refresh — count updates to "1 tasks · 1 done"
6. Run `cargo run -p unship -- docs list --type prd` — shows only PRDs
7. Run `cargo run -p unship -- docs list --type doc` — shows only regular docs
