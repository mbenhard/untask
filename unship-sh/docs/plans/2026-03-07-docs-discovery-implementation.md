...
**Goal:** Make docs discoverable out of the box by scanning both `.unship/docs/` and `docs/`, and add CLI commands to manage doc paths.

**Architecture:** Change default doc globs to include `docs/**/*.md`. Remove the hardcoded `DEFAULT_DOC_GLOB` merge pattern. Add `paths`, `add-path`, `remove-path` subcommands to `unship docs`. Update watcher to use config directly.
...
**Files:**
- Modify: `crates/unship-core/src/config.rs:9` (remove `DEFAULT_DOC_GLOB`)
- Modify: `crates/unship-core/src/config.rs:79-81` (update `default_docs()`)
...
```rust
// DELETE this line:
pub const DEFAULT_DOC_GLOB: &str = ".unship/docs/**/*.md";
```
...
Run: `cargo test -p unship-core 2>&1 | tail -20`
...
**Files:**
- Modify: `crates/unship-core/src/docs.rs:4` (remove `DEFAULT_DOC_GLOB` import)
- Modify: `crates/unship-core/src/docs.rs:106-116` (simplify `doc_patterns()`)
...
```rust
// Change this:
use unship_core::config::{Config, DEFAULT_DOC_GLOB};
// To this:
...
Run: `cargo test -p unship-desktop 2>&1 | tail -30`
Expected: Compiles. `ignores_markdown_outside_default_and_configured_docs` still passes (the test uses `notes/scratch.md` which doesn't match `docs/**/*.md`). `matches_default_docs_glob` still passes (uses `.unship/docs/plan.md`).
...
**Files:**
- Modify: `crates/unship-core/tests/docs_test.rs:84-103`
- Modify: `crates/unship-core/tests/config_test.rs:82`
...
fn list_uses_config_as_authoritative_source() {
    let tmp = setup();
    write_doc(&tmp, ".unship/docs/guide.md", "# Guide");
    write_doc(&tmp, "docs/architecture.md", "# Architecture");

    // Config only specifies docs/**/*.md — .unship/docs/ should NOT be searched
    let config_content = "docs:\n  - \"docs/**/*.md\"\n";
    std::fs::write(tmp.path().join(".unship/config.yml"), config_content).unwrap();
...
```rust
// Change this:
assert_eq!(config.docs, vec![".unship/docs/**/*.md"]);
// To this:
...
```bash
git add crates/unship-core/src/config.rs crates/unship-core/src/docs.rs crates/unship-core/tests/docs_test.rs crates/unship-core/tests/config_test.rs apps/desktop/src-tauri/src/watcher.rs
git commit -m "$(cat <<'EOF'
Change default doc globs to include docs/**/*.md

The default was only .unship/docs/**/*.md which meant projects with a
...
**Files:**
- Modify: `crates/unship-cli/src/cli.rs:136-145` (add new `DocsCommands` variants)
...
**Files:**
- Modify: `crates/unship-cli/src/commands/docs.rs` (add `paths`, `add_path`, `remove_path`)
...
pub fn remove_path(root: &Path, pattern: &str, json: bool) -> Result<()> {
    ...
    if json {
        ...
        if config.docs.is_empty() {
            println!("No doc paths configured. Use `unship docs add-path` to add one.");
        }
    }
...
**Files:**
- Modify: `crates/unship-cli/src/main.rs:115-120` (add match arms)
...
Run: `cargo test -p unship-cli -p unship-core 2>&1 | tail -10`
Expected: All tests pass.
...
Expected: `docs paths` shows the two defaults. `docs` lists actual docs from both `docs/` and `.unship/docs/`.

**Step 4: Commit**

```bash
git add crates/unship-cli/src/cli.rs crates/unship-cli/src/commands/docs.rs crates/unship-cli/src/main.rs
git commit -m "$(cat <<'EOF'
...
- unship docs add-path: add a glob pattern
- unship docs remove-path: remove a glob pattern
...
**Files:**
- Modify: `crates/unship-cli/tests/commands_test.rs` (add tests after docs section)
- Modify: `crates/unship-cli/src/cli.rs` (add parsing test)

**Step 1: Add CLI parsing tests**

In `crates/unship-cli/src/cli.rs`, add to the existing `tests` module:

```rust
#[test]
fn parses_docs_path_subcommands() {
    let paths = Cli::try_parse_from(["unship", "docs", "paths"]).unwrap();
    assert!(matches!(
        paths.command,
        Some(Commands::Docs { cmd: Some(DocsCommands::Paths) })
    ));

    let add = Cli::try_parse_from(["unship", "docs", "add-path", "specs/**/*.md"]).unwrap();
    assert!(matches!(
        add.command,
        Some(Commands::Docs { cmd: Some(DocsCommands::AddPath { pattern }) }) if pattern == "specs/**/*.md"
    ));

    let remove = Cli::try_parse_from(["unship", "docs", "remove-path", "docs/**/*.md"]).unwrap();
    assert!(matches!(
        remove.command,
...
In `crates/unship-cli/tests/commands_test.rs`, add after the existing docs tests:
...
fn docs_paths_shows_default_globs() {
    ...
    assert!(ok);
    assert!(stdout.contains(".unship/docs/**/*.md"));
...
fn docs_paths_json() {
    ...
    assert!(parsed.contains(&".unship/docs/**/*.md".to_string()));
...
Run: `cargo test -p unship-cli -p unship-core`
Expected: All tests pass.

**Step 4: Commit**

```bash
git add crates/unship-cli/src/cli.rs crates/unship-cli/tests/commands_test.rs
git commit -m "$(cat <<'EOF'
...
**Files:**
- Modify: `crates/unship-core/tests/docs_test.rs` (add test)
- Modify: `crates/unship-cli/tests/commands_test.rs` (add test)
...
```bash
git add crates/unship-core/tests/docs_test.rs crates/unship-cli/tests/commands_test.rs
git commit -m "$(cat <<'EOF'
