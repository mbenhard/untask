...
**Goal:** Add file attachment support to tasks — users can attach, view, and remove files (images, PDFs, text, etc.) from the task detail modal, with files stored locally in `.unship/attachments/`. Supports three input methods: file dialog, drag-and-drop, and CMD+V clipboard paste for images.

**Architecture:** Attachment metadata (filename, MIME type, size, timestamp) lives in the task's YAML frontmatter as a `Vec<AttachmentRef>`. Actual files are copied into `.unship/attachments/{task_id}/{filename}`. Tauri commands handle file copy/delete, frontend uses a Svelte 5 component integrated into `TaskModal.svelte`. Image attachments get inline thumbnails; other file types show an icon + filename. Clipboard paste uses the browser's native `paste` event to capture image blobs, sends raw bytes to a dedicated `attach_file_bytes` Tauri command that writes directly to disk.

**Tech Stack:** Rust (unship-core), Tauri 2 IPC + file dialog, Svelte 5 + Bits UI, Tailwind CSS (monochrome design language)
...
**Decision: `.unship/attachments/{task_id}/{filename}` with collision handling.**
...
- `.unship/attachments/` directory already exists (created by `init.rs:19`)
...
5. Rust side writes bytes to `.unship/attachments/{task_id}/{filename}`, creates `AttachmentRef`
...
1. **Git bloat** — Binary attachments in `.unship/attachments/` will bloat the git repo. Consider adding `.unship/attachments/` to `.gitignore` by default, or at least documenting the trade-off. Could offer a config option `attachments.gitignore: true`.

2. **File watcher events** — The desktop app uses `notify` to watch `.unship/` for changes. Copying files into `attachments/` will trigger watcher events. Ensure the watcher doesn't cause unnecessary reloads or handle attachment file events gracefully.
...
3. **Orphaned files** — If a task is deleted, its attachments should be cleaned up. The `TaskStore::delete()` method needs to also remove `.unship/attachments/{task_id}/`. If an attachment is removed from frontmatter, its file should also be deleted.

4. **Tauri asset protocol** — Need to register the asset scope in `tauri.conf.json` to allow the webview to access files in `.unship/attachments/`. This is a security-scoped access control in Tauri 2.
...
### New files
- `crates/unship-core/src/attachments.rs` — attachment CRUD operations
- `apps/desktop/src/lib/components/AttachmentList.svelte` — attachment list + drop zone UI

### Modified files
- `crates/unship-core/src/task.rs` — add `AttachmentRef` struct, `attachments` field
- `crates/unship-core/src/store.rs` — clean up attachments on task delete
- `crates/unship-core/src/lib.rs` — export `attachments` module
...
### Test files
- `crates/unship-core/tests/attachments_test.rs` — integration tests for attachment CRUD
...
**Files:**
- Modify: `crates/unship-core/src/task.rs`
...
**Files:**
- Create: `crates/unship-core/src/attachments.rs`
- Modify: `crates/unship-core/src/lib.rs`
...
Add `pub mod attachments;` to `crates/unship-core/src/lib.rs`.

**Step 3: Verify it compiles**

Run: `cargo check -p unship-core`
...
**Files:**
- Create: `crates/unship-core/tests/attachments_test.rs`
...
Run: `cargo test -p unship-core --test attachments_test`
...
pub fn attach_file(
    ...
    let att = unship_core::attachments::add_attachment(&root, id, &source)
...
pub fn attach_file_bytes(
    ...
    let att = unship_core::attachments::add_attachment_bytes(
...
pub fn delete_attachment(
    ...
    unship_core::attachments::remove_attachment(&root, id, &filename)
        .map_err(|e| e.to_string())?;
...
pub fn get_attachment_path(
    ...
    let root = require_project(&state)?;
    let path = unship_core::attachments::attachment_path(&root, id, &filename);
    if !path.is_file() {
...
Run: `cargo check -p unship-desktop`
...
In Tauri 2, the `asset` protocol needs to be configured to allow reading from the `.unship/attachments/` directory. This typically goes in the security section.
...
8. Delete the task — verify `.unship/attachments/{id}/` is cleaned up
...
1. **Should `.unship/attachments/` be gitignored by default?** Binary files will bloat the repo, but some users may want attachments tracked.
