...
| Rust core | `crates/unship-core/src/task.rs`, `crates/unship-core/src/store.rs`, `crates/unship-core/src/next.rs`, `crates/unship-core/src/types.rs` | implemented | Priority enum and task field were removed; lazy migration behavior is preserved because unknown frontmatter fields are ignored on read and omitted on write. |
| Rust CLI | `crates/unship-cli/src/cli.rs`, `crates/unship-cli/src/main.rs`, `crates/unship-cli/src/commands/list.rs`, `crates/unship-cli/src/output.rs` | implemented | Priority filter/sort/output paths were removed and help text was updated. |
| Tests and snapshots | `crates/unship-core/tests/next_test.rs`, `crates/unship-core/tests/store_test.rs`, `crates/unship-core/tests/task_test.rs`, `crates/unship-cli/tests/commands_test.rs`, `crates/unship-cli/tests/cli_snapshot_test.rs`, `crates/unship-cli/tests/snapshots/cli_snapshot_test__list_json.snap`, `crates/unship-cli/tests/snapshots/cli_snapshot_test__show_json.snap` | implemented | Planned coverage changes landed and the snapshots match the new JSON shape. |
| Docs | `docs/unship-design-language.md`, `CLAUDE.md`, `knowledgebase/index.html`, `knowledgebase/style.css`, `AGENTS.md` | implemented | Planned doc cleanup landed; review also removed stale priority references from static docs and repo instructions that were outside the original plan checklist. |
...
- Gaps:
  - No automated test covers the static `knowledgebase/` docs; review used a repository-wide `rg` scan to verify that only the intentional legacy `priority:` fixture in `crates/unship-core/tests/task_test.rs` remains outside plan docs.
