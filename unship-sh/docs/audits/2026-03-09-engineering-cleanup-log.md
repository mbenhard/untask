# Unship Engineering Cleanup Log
...
- Added shared column workflow orchestration in `unship-core/src/columns.rs`.
...
- Routed desktop doc saves through `unship-core` instead of raw Tauri-side file writes.
- Applied project locking to docs create/rename/move/delete/save operations.
- Fixed existing clippy issues in `unship-core` and one in CLI dispatch.
...
- Added an internal `scan_tasks` helper in `crates/unship-core/src/store.rs` so aggregate/task-migration paths no longer pay the sort-and-full-list cost of `list(None)`.
...
- Intentionally did not move `.unship/`, `test-dir/.unship/`, or `docs/plans/` yet because that is a repository-organization decision rather than a safe unilateral code cleanup.
...
- Approve or reject the repository-boundary proposal for `.unship/`, `test-dir/.unship/`, and `docs/plans/`.
