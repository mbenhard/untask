# Unship Design Language Spec

This document reverse-engineers the shared design language of:

- The Unship desktop app
- The main website at `https://unship.sk/`
...
Unship feels like a native, keyboard-first macOS utility that happens to be modern, not a trendy SaaS product.
...
Unship is not spacious in the typical consumer-web sense. It is compact:
...
Unship is "industrial macOS minimalism with a tiny bird living inside it."
...
Unship uses opacity constantly instead of extra colors:
...
Fallback product font choices in the app exist, but Unship's identity is clearly strongest with Geist + Geist Mono.
...
- Keep headings compact. Unship's notes headings are intentionally smaller than a document editor would normally make them.
...
Unship does not use oversized radii.
...
Unship empty states are sparse:
...
Unship's copy is as important as its visuals.
...
Unship is refined industrial, not deliberately ugly industrial.
...
If you want a new app to feel like Unship immediately, start with this formula:
...
Unship's polish comes from precision and constraint, not from visual abundance.
