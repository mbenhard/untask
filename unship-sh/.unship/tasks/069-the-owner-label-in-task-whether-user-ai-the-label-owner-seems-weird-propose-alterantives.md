...
Analyzed how the `owner` field is used across the full stack: Rust data model (`task.rs`, `store.rs`, `next.rs`), Tauri commands (`commands.rs`), TypeScript types (`api.ts`), the task modal UI (`TaskModal.svelte`), kanban cards (`Kanban.svelte`), and the CLI skill instructions (`unship.md`). The field is a simple binary toggle — `”user”` means human-only, `null` means AI-eligible.
