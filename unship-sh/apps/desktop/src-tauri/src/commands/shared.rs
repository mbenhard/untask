...
use unship_core::docs::{DocType, DocsStore};
use unship_core::store::{AttachmentTextPreview, TaskStore};
...
pub(crate) fn ensure_project_dir(path: &Path) -> Result<(), String> {
    if path.join(".unship").is_dir() {
        Ok(())
    } else {
        Err(format!("not an unship project: {}", path.display()))
    }
}
...
pub(crate) fn columns_to_dto(config: &unship_core::config::Config) -> Vec<ColumnDto> {
    config
        .columns
        .iter()
        .map(|c| ColumnDto {
...
mod tests {
    use super::*;
    use unship_core::store::TaskUpdate;
    ...
    fn relative_project_path_prefers_project_relative_doc_references() {
        ...
        let doc = root.join(".unship/docs/guide.md");

        assert_eq!(
            relative_project_path(root, &doc),
            ".unship/docs/guide.md".to_string()
        );
    }
