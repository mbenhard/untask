<script lang="ts">
  ...
  function classifyRefreshTargets(changedPaths: string[]): RefreshTargets {
    const configChanged = changedPaths.includes(".unship/config.yml");
    if (configChanged) {
      return { config: true, tasks: true, docs: true };
    }

    const tasksChanged = changedPaths.some((path) => path.startsWith(".unship/tasks/"));
    const docsChanged = changedPaths.some((path) => !path.startsWith(".unship/tasks/"));
  ...
  function affectsDocsExternally(paths: string[]) {
    return paths.some((path) => path === ".unship/config.yml" || !path.startsWith(".unship/tasks/"));
  }
