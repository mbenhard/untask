<script lang="ts">
  ...
  async function chooseFolder() {
    const selection = await open({
      directory: true,
      multiple: false,
      title: "Choose an Unship project",
    });
    ...
    try {
      ...
      const msg = String(e);
      if (msg.includes("not an unship project")) {
...
{#if mode === "dropdown"}
  ...
  >
    <Dialog.Portal>
      <Dialog.Overlay class="fixed inset-0 z-40 bg-black/30 backdrop-blur-[2px]" />
      <!-- svelte-ignore a11y_no_static_element_interactions -->
      <Dialog.Content
        class="fixed inset-x-0 top-12 z-40 mx-auto w-full max-w-[320px] rounded-[6px] border border-border/60 bg-card shadow-[0_12px_36px_-8px_rgba(0,0,0,0.4)]"
        style="animation: picker-in 150ms ease-out both"
        onkeydown={handleKeydown}
      >
        <Dialog.Title class="sr-only">Switch project</Dialog.Title>
    ...
    <div class="w-full max-w-[320px] space-y-4 px-4">
      <div>
        ...
        <p class="mt-1 text-[12px] leading-[1.45] text-muted-foreground">
          Choose a directory containing an <span class="font-mono">.unship/</span> folder,
      ...
      {#if initPrompt}
        <div class="rounded-[6px] border border-border/60 bg-card/80 p-3">
          <p class="text-[12px] text-foreground">
            No <span class="font-mono">.unship/</span> directory found.
          </p>
          <p class="mt-1 text-[12px] text-muted-foreground">
            Initialize unship in <span class="font-mono text-[11px]">{initPrompt}</span>?
          </p>
