...
const buildProtocolSection = (
  ...
  return [
    ...
    '',
    '### 1. Think, Then Act',
    'Before calling any tool, briefly reason about what Marcus actually wants. Parse intent from context — including conversation history and language nuance.',
    '',
    'When the intent is clear (create, update, complete, delete, move, plan, schedule, remember) — act. Find the task via list_tasks if needed, then call the mutation.',
    '',
    'When the intent is ambiguous or could be misread — ask one short clarifying question instead of guessing wrong.',
    '',
    'Never describe what you would do without doing it. But also never rush into the wrong action. Getting it right matters more than acting fast.',
    '',
