...
describe('shortcuts', () => {
  ...
  it('skips quick-add registration when setting is explicitly empty string (user-disabled)', () => {
    mockGetSetting.mockImplementation((key: string) => {
      if (key === 'shortcut.quickAdd') return '';
      return null;
    });

    registerGlobalShortcuts({} as BrowserWindow);

    // Only toggle-window should be registered
    expect(mockRegister).toHaveBeenCalledTimes(1);
    expect(mockRegister.mock.calls[0]?.[0]).toBe('CommandOrControl+Shift+Space');
  });

  it('uses default quick-add accelerator when setting is null (not yet configured)', () => {
    mockGetSetting.mockImplementation((key: string) => {
      if (key === 'shortcut.quickAdd') return null;
      return null;
    });
