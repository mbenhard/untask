...
<div class="flex min-h-0 flex-1 overflow-hidden">
  <aside class="flex w-[280px] min-w-[280px] flex-col border-r border-border/80">
    ...
    >
      ...
      {:else}
        {#each flatNodes as item}
          <div
            class={cn(
              "flex min-h-[36px] items-center border-b border-border/30",
              selectedPath === item.node.node_path && "bg-accent/50",
            )}
            style={`padding-left: ${12 + item.depth * 14}px;`}
          >
            {#if item.node.kind === "doc"}
              <span class="ml-3 w-4 shrink-0"></span>
            {:else}
              <button
                type="button"
                class="ml-3 flex h-4 w-4 shrink-0 items-center justify-center rounded-[4px] border border-border/60 font-mono text-[9px] text-muted-foreground hover:bg-accent"
                onclick={() => onToggle(item.node)}
              >
                {expandedPaths.has(item.node.node_path) ? "−" : "+"}
              </button>
            {/if}

            <button
              type="button"
              class="flex min-w-0 flex-1 items-center gap-1.5 px-2 py-1.5 text-left hover:bg-accent/40"
              onclick={() => onNodeSelect(item.node)}
            >
              <div class="min-w-0 flex-1">
                <p class="truncate text-[12px] text-foreground">{item.node.name}</p>
                {#if item.node.kind === "root" && item.node.read_only}
  ...
  <section class="flex min-w-0 flex-1 flex-col">
    {#if selectedNode}
      <div class="border-b border-border/80 px-4 py-2.5">
        <div class="flex items-center justify-between gap-4">
          <div class="min-w-0">
            <p class="truncate font-mono text-[10px] uppercase tracking-[0.08em] text-muted-foreground">
              {renderActionTitle(selectedNode)}
            </p>
        ...
        {#if actionMode}
          <div class="mt-3 flex flex-wrap items-center gap-2 border-t border-border/50 pt-3">
            {#if actionMode === "move"}
              <select
                bind:value={moveDestination}
                class="min-w-[220px] rounded-[4px] border border-border bg-background px-2 py-1 font-mono text-[11px] text-foreground outline-none focus:border-ring"
              >
            ...
            {:else}
              <input
                ...
                class="min-w-[220px] rounded-[4px] border border-border bg-background px-2 py-1 font-mono text-[11px] text-foreground placeholder:text-muted-foreground/40 outline-none focus:border-ring"
            ...
            <button
              type="button"
              class="rounded-[4px] border border-border px-2 py-1 font-mono text-[10px] text-foreground transition-colors hover:bg-accent disabled:opacity-50"
            ...
            <button
              type="button"
              class="rounded-[4px] border border-border px-2 py-1 font-mono text-[10px] text-muted-foreground transition-colors hover:bg-accent hover:text-foreground"
      ...
      <div class="flex min-h-0 flex-1 flex-col">
        <div class="border-b border-border/80 px-4 py-3">
          <div class="flex items-start justify-between gap-4">
            <div class="min-w-0">
              <p class="truncate text-[13px] font-medium text-foreground">{selectedNode.name}</p>
              <div class="mt-1 flex flex-wrap items-center gap-2">
                <span class="font-mono text-[10px] text-muted-foreground">
                  {selectedNode.relative_path}
                </span>
                <span class="rounded-[4px] border border-border/70 px-1.5 py-0.5 font-mono text-[10px] text-muted-foreground">
                  {folderKindLabel(selectedNode)}
                </span>
                <span class="rounded-[4px] border border-border/70 px-1.5 py-0.5 font-mono text-[10px] text-muted-foreground">
            ...
            {#if selectedBreadcrumbs.length > 0}
              <div class="hidden max-w-[320px] items-center gap-1 overflow-hidden font-mono text-[10px] text-muted-foreground md:flex">
                {#each selectedBreadcrumbs as segment, index}
                  <span class="truncate">{segment}</span>
                  {#if index < selectedBreadcrumbs.length - 1}
                    <span>/</span>
                  {/if}
                {/each}
              </div>
          ...
          {#if selectedChildren.length === 0}
            <div class="flex h-full items-center justify-center px-6 py-12">
              <div class="text-center">
                <p class="font-mono text-[11px] text-muted-foreground/60">Empty folder</p>
                <p class="mt-2 text-[12px] text-muted-foreground/40">
                  {selectedNode.read_only
                    ? "This source is browse-only."
                    : "Create a document or folder to start structuring this space."}
                </p>
              </div>
            </div>
          {:else}
            {#each selectedChildren as child}
              <button
                type="button"
                class="flex min-h-[40px] w-full items-center justify-between border-b border-border/30 px-4 py-2 text-left transition-colors hover:bg-accent/40"
                onclick={() => onNodeSelect(child)}
              >
                <div class="min-w-0 flex-1">
                  <p class="truncate text-[13px] text-foreground">{child.name}</p>
                  <p class="truncate font-mono text-[10px] text-muted-foreground">
                    {child.relative_path}
                  </p>
                </div>
                <span class="ml-4 font-mono text-[10px] uppercase tracking-[0.06em] text-muted-foreground">
                  {child.kind}
                </span>
              </button>
            {/each}
    ...
    {:else}
      <div class="flex flex-1 items-center justify-center">
        <span class="font-mono text-[11px] text-muted-foreground">No source selected</span>
      </div>
    {/if}
  </section>
</div>
