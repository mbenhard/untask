# Untask Engineering Audit

Date: 2026-03-09
Reviewer: Codex
Status: Complete
Scope: Full repository audit across `untask-core`, `untask-cli`, desktop/Tauri app, tests, scripts, tooling, docs-adjacent engineering artifacts, and local developer workflows.

## Project Overview

Untask is a local-first task/document workflow project implemented as:

- A Rust workspace with shared domain and storage logic in `crates/untask-core`
- A Rust CLI in `crates/untask-cli`
...
- Read the full tracked Rust test surface for `untask-core`, `untask-cli`, and desktop Tauri backend
...
- `crates/untask-core`: local data model, filesystem-backed storage, task/docs operations, project discovery, and supporting utilities
- `crates/untask-cli`: thin command layer over `untask-core` with human/json output modes
...
- The `untask-core` crate is the correct place for domain and persistence logic, but several cross-cutting workflows still leak into CLI and Tauri layers.
...
- Where it appears:
  - `crates/untask-cli/src/commands/column.rs`
  - `apps/desktop/src-tauri/src/commands.rs`
  - `crates/untask-core/src/store.rs`
...
- Recommended improvement: Move column orchestration into `untask-core` and require destination validation against canonical config before any task writes occur.
- Priority level: P1
...
- Where it appears:
  - `crates/untask-core/src/next.rs`
...
- Where it appears:
  - `apps/desktop/src-tauri/src/commands.rs`
- Why it matters: This makes desktop backend changes expensive, obscures ownership boundaries, and discourages extraction of reusable behavior into `untask-core`.
...
- Where it appears:
  - `crates/untask-core/src/config.rs`
  - `apps/desktop/src/lib/utils.ts`
  - `apps/desktop/src-tauri/src/commands.rs`
  - `apps/desktop/src/lib/api.ts`
  - `crates/untask-core/src/task.rs`
...
- Recommended improvement: Push more normalization/translation into `untask-core`, minimize bespoke Tauri DTOs where possible, and introduce generated or single-source contract definitions for desktop APIs.
- Priority level: P2
...
- Where it appears:
  - `crates/untask-core/src/fs.rs`
  - `crates/untask-core/src/store.rs`
  - `crates/untask-core/src/docs.rs`
  - `apps/desktop/src-tauri/src/commands.rs`
...
- Recommended improvement: Introduce a docs write path in `untask-core` that uses the same durability and locking policy as task writes, or explicitly document and isolate why docs are different.
- Priority level: P2
...
- Description: The repo tracks live `.untask` task data, binary attachments under `.untask/attachments/`, a second `test-dir/.untask` project, and a large `docs/plans/` archive. This increases noise during engineering work and makes it harder to separate product code from local/project state.
- Where it appears:
  - `.untask/**`
  - `test-dir/.untask/**`
  - `docs/plans/**`
...
- Status normalization exists in both `untask-core` and frontend utilities.
...
- Repository tracking of live `.untask` state, binary attachment examples, `test-dir/.untask` fixtures, and archived plans adds context bloat unrelated to runtime code paths.
...
- Remove the two clippy violations in `crates/untask-core/src/store.rs`.
- Standardize desktop package manager usage and CI cache/install commands.
- Decide whether tracked `.untask` project data, attachment binaries, and `test-dir/.untask` belong in the main repo or should move to explicit fixtures/examples.
...
- Create a docs persistence API in `untask-core` with locking and atomic write policy.
...
- Local `.untask` data inside the repository and `test-dir/` may be development fixtures rather than committed product assets.
...
1. Move column rename/delete orchestration into `untask-core`.
...
- `cargo clippy --workspace --all-targets -- -D warnings`: failed on `clippy::needless_borrow` in `crates/untask-core/src/store.rs:279` and `crates/untask-core/src/store.rs:317`
...
That execution pass fixed the P1 column/delete and `next` correctness issues, moved column workflow orchestration into `untask-core`, hardened docs persistence, reduced some frontend dead/duplicate surface, and brought current verification back to green (`cargo test`, `cargo clippy -D warnings`, `npm run check`).
