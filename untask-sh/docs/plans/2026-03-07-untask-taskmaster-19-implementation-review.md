...
- `docs/plans/2026-03-06-untask-implementation.md` (`Task 19`)
...
- P2: The new CI workflow validated desktop typecheck, clippy, and build, but it skipped the desktop Rust unit tests entirely. That left the watcher filtering/debounce logic unguarded in CI. Fixed by adding `cargo test -p untask-desktop` to `.github/workflows/ci.yml`.
...
- Before:
  - `cargo test -p untask-desktop` -> passed (`6 passed`)
  - `cargo test --workspace --exclude untask-desktop` -> passed
  - `npm run check` -> passed
- After:
  - `cargo fmt --all` -> passed
  - `cargo test -p untask-desktop` -> passed (`15 passed`)
  - `cargo clippy -p untask-desktop --all-targets -- -D warnings` -> passed
  - `cargo test --workspace --exclude untask-desktop` -> passed
  - `npm run check` -> passed
...
- `cargo fmt --all`
- `cargo test -p untask-desktop`
- `cargo clippy -p untask-desktop --all-targets -- -D warnings`
- `cargo test --workspace --exclude untask-desktop`
- `npm run check`
- `npm run tauri build`
