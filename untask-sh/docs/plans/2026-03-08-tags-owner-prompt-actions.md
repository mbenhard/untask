...
**Tech Stack:** Rust (untask-core), Tauri commands, TypeScript, Svelte 5, Bits UI (popover), Tailwind CSS
...
**Files:**
- Modify: `crates/untask-core/src/task.rs:9-36` (Task struct, TaskFrontmatter, From impl)
- Modify: `crates/untask-core/src/store.rs:20-28` (TaskUpdate struct)
- Modify: `crates/untask-core/src/store.rs:208-244` (update function)

**Step 1: Add `owner` field to Task struct**

In `crates/untask-core/src/task.rs`, add to Task struct after `confidence`:
...
In `crates/untask-core/src/store.rs`, add to `TaskUpdate`:
...
Run: `cargo build -p untask && cd apps/desktop && npx svelte-check --tsconfig ./tsconfig.json`
...
### Task 7: Owner Field — CLI Filter in `untask next`

**Files:**
- Modify: `crates/untask-core/src/next.rs:44-48` (filter out user-owned tasks)
...
Create a task with `owner: user` in frontmatter, run `cargo run -p untask -- next --json`, verify it doesn't appear in open_tasks.

**Step 4: Commit**

```
feat: untask next filters out user-owned tasks
```
...
<SplitButton
  ...
  defaultAction="untask-prompt-mode"
  onAction={copyPrompt}
/>
```
