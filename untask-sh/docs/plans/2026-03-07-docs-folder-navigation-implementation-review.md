...
- Task 1 `implemented`
  - Deterministic writable-root inference, browse-only degradation, tree DTOs, and safe file-management primitives are implemented in `crates/untask-core/src/docs.rs`.
...
- Before:
  - `cargo test -p untask-core --test docs_test` passed (`19 passed`)
  - `cargo test -p untask-desktop` passed (`17 passed`)
