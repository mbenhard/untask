...
Columns are per-project, stored in `.untask/config.yml`. Users pick a preset or create custom columns at init time. Columns can be added, renamed, reordered, and deleted after setup — from both CLI and desktop.
...
Add `Config::save()` to write config back to `.untask/config.yml`. Currently config is read-only — all column operations need this.
...
2. `init()` creates directories (existing behavior) AND writes chosen columns to `.untask/config.yml` (new behavior)
...
- If `.untask/config.yml` exists with columns, skip the preset picker
- `untask init --reset-columns` to re-pick
...
- CLI: `untask column add <name> [--after <column>] [--done]`
...
- CLI: `untask column rename <old> <new>`
- Reject if new name collides with existing ID or alias

### Reorder

- Move column to new position
- CLI: `untask column move <name> --after <column>` or `--before <column>`
- Desktop: drag-and-drop on column headers
- Config-only change, no task files modified

### Delete

- CLI: `untask column delete <name> [--move-to <column>] [--delete-tasks]`
...
Idempotent for directories. If `.untask/config.yml` exists, don't overwrite unless `--reset-columns`. Existing configs without the `done` field default to `false` via `#[serde(default)]` — backward compatible.

### Desktop first launch

No `.untask/` directory = show preset picker inline before loading board. Config exists = load directly.
