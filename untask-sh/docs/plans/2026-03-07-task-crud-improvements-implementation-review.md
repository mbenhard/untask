...
| Manual ordering via `position` | `apps/desktop/src/lib/api.ts`, `apps/desktop/src-tauri/src/commands.rs`, `crates/untask-core/src/task.rs`, `crates/untask-core/src/store.rs` | partial | `position` is modeled end-to-end, but create/update logic does not satisfy the default-position and migration rules from the plan. |
