...
- Taskmaster 6 (`Repair Command Implementation`): implemented in [crates/untask-core/src/repair.rs](/Users/marcusbenhard/Development/untitled/untask-sh/crates/untask-core/src/repair.rs) with read-only scanning, write-path repair, locking, and explicit action reporting. Review changes closed the status-normalization gap so alias statuses now repair to canonical column IDs instead of being left behind.
- Taskmaster 16 (`Desktop App Scaffold with Tauri 2 and Svelte 5`): implemented across [apps/desktop/src/App.svelte](/Users/marcusbenhard/Development/untitled/untask-sh/apps/desktop/src/App.svelte), [apps/desktop/src/app.css](/Users/marcusbenhard/Development/untitled/untask-sh/apps/desktop/src/app.css), [apps/desktop/src/main.ts](/Users/marcusbenhard/Development/untitled/untask-sh/apps/desktop/src/main.ts), [apps/desktop/src/lib/components/ScaffoldPanel.svelte](/Users/marcusbenhard/Development/untitled/untask-sh/apps/desktop/src/lib/components/ScaffoldPanel.svelte), and [apps/desktop/src-tauri/src/lib.rs](/Users/marcusbenhard/Development/untitled/untask-sh/apps/desktop/src-tauri/src/lib.rs). Review changes replaced the stock Tauri greet placeholder with a design-language-consistent shell, frontend dialog wiring, and self-hosted Geist fonts.

## Findings (by severity)

- P1: Repair only rewrote unknown statuses and left valid aliases like `to-do` untouched, so Taskmaster 6 was not actually normalizing statuses to canonical column IDs on the bulk repair path. Fixed in [crates/untask-core/src/repair.rs](/Users/marcusbenhard/Development/untitled/untask-sh/crates/untask-core/src/repair.rs#L13) and covered with new regressions in [crates/untask-core/tests/repair_test.rs](/Users/marcusbenhard/Development/untitled/untask-sh/crates/untask-core/tests/repair_test.rs#L87).
...
## Test Delta
- Before:
  - `cargo test -p untask-core --test repair_test -- --nocapture` -> 10 passed
  - `cargo build --workspace` -> passed
  - `cd apps/desktop && npm run check` -> passed
- After:
  - `cargo test -p untask-core --test repair_test -- --nocapture` -> 12 passed
  - `cargo build --workspace` -> passed
...
- `cargo fmt --all`
- `cargo test -p untask-core --test repair_test -- --nocapture`
