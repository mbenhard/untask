...
| --- | --- | --- | --- |
| 1. Add `AttachmentRef` and `Task.attachments` | implemented | `crates/untask-core/src/task.rs` | Metadata is serialized through task frontmatter. |
| 2. Create attachments CRUD module | implemented | `crates/untask-core/src/attachments.rs`, `crates/untask-core/src/lib.rs` | Add/remove/path helpers exist, including byte uploads. |
| 3. Clean up attachments on task delete | implemented | `crates/untask-core/src/store.rs` | Single-task and bulk delete paths remove attachment dirs. |
| 4. Add attachment integration tests | implemented | `crates/untask-core/tests/attachments_test.rs` | Core file lifecycle tests are present and passing. |
...
| 11. Update `.gitignore` handling | implemented | `crates/untask-core/src/init.rs` | New projects now ignore `.untask/attachments/`. |
| 12. End-to-end testing and polish | partial | `cargo test -p untask-core --test attachments_test`, `cargo test -p untask-desktop`, `pnpm --dir apps/desktop check` | Scoped verification exists, but no UI/E2E coverage was added for attach dialog, paste, preview, or drag-and-drop. |
...
1. The asset protocol scope is wider than the approved design.
   - Plan evidence: Task 10 narrows access to `["**/.untask/attachments/**"]`.
...
## Test Delta
- Before: `cargo test -p untask-core attachments -- --nocapture` compiled successfully but matched zero tests, so it did not provide a meaningful baseline for the attachment flow.
- After: `cargo test -p untask-core --test attachments_test -- --nocapture` passed (5/5), `cargo test -p untask-desktop -- --nocapture` passed (17/17), and `pnpm --dir apps/desktop check` passed with 0 errors and 1 unrelated accessibility warning in `TaskModal.svelte`.
...
- `cargo test -p untask-core attachments -- --nocapture`
