...
- Task 1 implemented: task PRD linkage added in `crates/untask-core/src/task.rs` and `crates/untask-core/src/store.rs`, with round-trip and linkage-count coverage in `crates/untask-core/tests/store_test.rs`.
- Tasks 2-3 implemented: doc frontmatter parsing, `DocType`, and tree exposure landed in `crates/untask-core/src/docs.rs`, with parsing/tree coverage in `crates/untask-core/tests/docs_test.rs`.
- Task 4 implemented: PRD-linked task counting shipped in `crates/untask-core/src/store.rs` and is covered in `crates/untask-core/tests/store_test.rs`.
- Task 5 implemented: CLI `add --prd` and `docs list --type` shipped in `crates/untask-cli/src/cli.rs`, `crates/untask-cli/src/commands/add.rs`, `crates/untask-cli/src/commands/docs.rs`, and `crates/untask-cli/src/main.rs`.
...
- Manual CLI smoke test in a temp project:
  - `untask init`
  - write `.untask/docs/spec.md` with `type: prd`
  - `untask add "Build feature" --prd .untask/docs/spec.md`
  - `untask docs list --type prd`
  - `untask --json show 1`
