...
Untask is a local-first project companion. All durable data lives inside `.untask/` at the repository root and is shared across two access layers:

1. **CLI** for humans, scripts, and AI agents.
2. **macOS app** for board, task detail, docs, and richer editing.

An Untask skill teaches agents when to call the CLI and how to keep task state current.
...
2. Mutations acquire `.untask/.lock`.
...
2. The macOS app reads and writes the same `.untask/` data as the CLI.
