...
1. Create `crates/untask-core/src/error.rs`:
```rust
use thiserror::Error;

#[derive(Error, Debug)]
pub enum UntaskError {
    #[error("IO error: {0}")]
    ...
    #[error("Project not initialized - run 'untask init' first")]
    NotInitialized,
...
pub type Result<T> = std::result::Result<T, UntaskError>;
```

2. Create `crates/untask-core/src/types.rs` with Theme enum (`mono`, `color`, `none`), Priority enum, and shared DTOs.

3. Create `crates/untask-core/src/config.rs`:
...
Create `crates/untask-core/tests/config_test.rs`:
