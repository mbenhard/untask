...
id: 52
title: |-
  The core problem: /untask covers session start and task
    pickup, but says nothing about what to do when finishing. The agent has to independently
    remember to invoke /untask-finish — which is easy to forget since it's never referenced.

    Three approaches:

    A) Merge finish into /untask — Add a "Completing a task" section to the main skill. The
    ...
    B) Keep separate, add a pointer — Add "When done, invoke /untask-finish" to /untask.
