...
status: review
created: 2026-03-08
updated: 2026-03-08T15:04:50.756681Z
position: 1.0
---
...
Analyzed the current confidence presentation across three UI surfaces (Kanban cards, TaskModal, ReviewView). The field is either absent (Kanban) or styled identically to other neutral metadata (modal/review), making it effectively invisible. Proposed three approaches — dot-based, banner-based, and a hybrid — all compliant with the monochrome/restrained design language. Recommended the hybrid approach (Approach C) which shows a ring-style confidence dot on cards only for low/medium confidence, keeping high-confidence tasks visually clean, and adds tinted borders to the modal chip. This follows the established pattern of "tiny dots for state signals" from the design language spec.
