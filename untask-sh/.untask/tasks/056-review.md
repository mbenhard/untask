...
### What exists today
- `crates/untask-core/src/git.rs` already uses `std::process::Command::new("git")` for read-only operations (branch, log, status)
...
1. Calls `git add .untask/` (only stage task file changes, not unrelated code)
...
|----------|-----------|----------|
| AI agent already committed | `git status .untask/` shows clean | Show "Approve" only (no changes to commit) |
| Human processed it (no agent) | Same — check if .untask/ has uncommitted changes | Same logic |
| Mixed: some task files changed, some not | `git status .untask/tasks/<id>*` | Only commit the specific task file |
| Uncommitted code changes alongside | N/A — we only `git add .untask/` | Code changes stay unstaged, safe |
...
- **"Approve & Commit"** — marks done + commits `.untask/` changes
- Gray out "& Commit" when `git status .untask/` shows no changes
