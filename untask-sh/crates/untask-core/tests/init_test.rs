...
use untask_core::fs::atomic_write;
use untask_core::init::init;
use untask_core::lock::ProjectLock;
...
fn init_creates_all_directories() {
    ...
    assert!(dir.path().join(".untask/attachments").is_dir());
    assert!(dir.path().join(".untask/cache").is_dir());
}
...
fn lock_requires_initialized_project() {
    ...
    match ProjectLock::acquire(dir.path()) {
        Err(UntaskError::NotInitialized) => {}
        Err(other) => panic!("expected NotInitialized, got {other:?}"),
        Ok(_) => panic!("expected lock acquisition to fail without .untask"),
    }
}
