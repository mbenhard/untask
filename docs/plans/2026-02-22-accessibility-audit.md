...
**F-1: Chat overlay panel has no focus trap** — FIXED
...
- **Resolution:** Already fixed — `useFocusTrap(openPanelRef, chatOverlayState === 'open')` applied at line 102.
- **WCAG:** 2.4.3 Focus Order

### MEDIUM

**F-2: Focus restoration after chat overlay close is fragile** — FIXED
...
- **Resolution:** Added `useEffect` on `activeView` that focuses `[data-primary-focusable]` after view change.
- **WCAG:** 2.4.3
...
**A-1: TitleBar navigation lacks tab semantics** — FIXED
...
- **Resolution:** Added `role="tablist"` to nav, `role="tab"` and `aria-selected` to each button.
- **WCAG:** 4.1.2

**A-2: Settings view tab strip lacks tab semantics** — FIXED
...
- **Resolution:** Added `role="tablist"` to nav, `role="tab"` and `aria-selected` to buttons, `role="tabpanel"` to content area.
- **WCAG:** 4.1.2

**A-3: Custom note context menu lacks `role="menu"`** — FIXED
...
- **Resolution:** Added `role="menu"` to container and `role="menuitem"` to all 7 buttons.
- **WCAG:** 4.1.2

**A-4: Task action popover lacks menu role** — FIXED
- `TaskItem.tsx:583-715` — Buttons inside PopoverContent function as menu but lack ARIA semantics.
- **Fix:** Add `role="menu"` and `role="menuitem"`.
- **Resolution:** Added `role="menu"` to main and project views, `role="menuitem"` to all buttons.
- **WCAG:** 4.1.2

### MEDIUM

**A-5: Priority dot indicator not accessible** — FIXED
...
- **Resolution:** Added sr-only span with priority text.
- **WCAG:** 1.3.1, 1.4.1

**A-6: Search results lack combobox/listbox pattern** — FIXED
...
- **Resolution:** Added `role="combobox"`, `aria-expanded`, `aria-controls` on input; `id` + `role="listbox"` on results; `role="option"` + `aria-selected` on each result.
- **WCAG:** 4.1.2, 1.3.1

**A-7: Thread list lacks selection semantics** — FIXED
...
- **Resolution:** Added `role="listbox"` with `aria-label="Threads"` to container; `role="option"` + `aria-selected` to each thread.
- **WCAG:** 4.1.2

### LOW

**A-8: Chat "Back to threads" button lacks descriptive label** — FIXED
- `AppShell.tsx:382-391` — Button with arrow icon has no `aria-label`.
- **Fix:** Add `aria-label="Back to threads"`.
- **Resolution:** Added `aria-label="Back to threads"` and `aria-hidden="true"` on icon.
- **WCAG:** 4.1.2
...
**S-1: Toast notifications not announced** — FIXED
...
- **Resolution:** Already fixed — `role="status"` and `aria-live="polite"` present on Toast.
- **WCAG:** 4.1.3

### HIGH

**S-2: Task completion state changes not announced** — FIXED
...
- **Resolution:** Added `aria-checked={isCompleted}` to both completion toggle buttons.
- **WCAG:** 4.1.3
...
**S-3: Decorative icons not consistently hidden** — FIXED
...
- **Resolution:** Added `aria-hidden="true"` to ~60 decorative Lucide icons across TaskItem, TaskBody, SearchModal, ThreadListView, AppShell, NotesList, ChatInput, ChatView, UpdateBanner, SectionGroup, TaskDueDatePicker, and UntaskFormattingToolbar.
- **WCAG:** 1.3.1
...
**K-1: Note hover actions are mouse-only** — FIXED
...
- **Resolution:** Added `group-focus-within:opacity-100` to all 3 hover-action containers.
- **WCAG:** 2.1.1

**K-2: Thread hover actions are mouse-only** — FIXED
- `ThreadListView.tsx:271-296` — Archive and delete buttons same pattern as K-1.
- **Fix:** Add `group-focus-within:opacity-100`.
- **Resolution:** Added `group-focus-within:opacity-100` to the thread actions container.
- **WCAG:** 2.1.1
...
**K-4: Tab key trapped within task lists** — FIXED
...
- **Resolution:** Removed `preventDefault()` on Tab. Roving tabIndex pattern (`tabIndex={isFocused ? 0 : -1}`) ensures arrow keys handle item navigation while Tab naturally leaves the list.
- **WCAG:** 2.1.1, 2.4.3

### LOW

**K-5: InlineTaskInput metadata not keyboard-navigable in list context** — FIXED
- `InlineTaskInput.tsx:197-242` — Tab blocked by task list keyboard handler.
- **Resolution:** Fixed by K-4 — Tab is no longer trapped in task lists.
- **WCAG:** 2.1.1
...
**C-1: `muted-foreground` fails AA contrast** — FIXED
...
- **Resolution:** Updated dark `--muted-foreground` to `#999999` and light to `#636363`.
- **WCAG:** 1.4.3

**C-2: Opacity modifiers further reduce contrast** — PARTIALLY FIXED
...
- **Resolution:** Updated `SEGMENT_EMPTY` from `/50` to `/70`. Base muted-foreground colors improved, so all opacity variants now have better contrast. Decorative uses (separator dots, drag handles) left as-is.
- **WCAG:** 1.4.3

### MEDIUM

**C-3: Placeholder text contrast insufficient** — FIXED
...
- **Resolution:** Updated all placeholder opacity from `/30` and `/40` to `/50` across InlineTaskInput, QuickAddApp, QuickAddDueDatePicker, TaskList, and SearchModal.
- **WCAG:** 1.4.3 (informative)

### LOW

**C-4: Disabled state uses `opacity-40`** — FIXED
...
- **Resolution:** Changed `opacity-40` to `opacity-50` in both occurrences.
...
**X-1: `<html>` lacks `lang` attribute** — FIXED
- `index.html:2` — Screen readers can't determine page language.
- **Fix:** Add `lang="en"`.
- **Resolution:** Already fixed — `lang="en"` present on both `index.html` and `quick-add.html`.
- **WCAG:** 3.1.1
...
**E-2: Chat error messages not announced** — FIXED
- `ChatView.tsx:694-718` — Error div lacks `role="alert"`. Renders outside the `aria-live` region.
- **Fix:** Add `role="alert"`.
- **Resolution:** Already fixed — `role="alert"` present on error div.
- **WCAG:** 4.1.3

### MEDIUM

**E-3: Chat input image error not announced** — FIXED
- `ChatInput.tsx:235-237` — Error `<p>` has no live region.
- **Fix:** Add `role="alert"`.
- **Resolution:** Added `role="alert"` to error paragraph.
- **WCAG:** 4.1.3
...
**E-4: Shortcut recorder errors not announced** — FIXED
- `SettingsShortcuts.tsx:248-249` — Conflict/validation errors not in live region.
- **Fix:** Add `role="alert"`.
- **Resolution:** Added `role="alert"` to error span.

**X-4: Metadata segments only show field name, not value** — FIXED
- `TaskBody.tsx` — `aria-label="Status"` instead of `aria-label="Status: Active"`.
- **Fix:** Include current value.
- **Resolution:** Updated StatusSegment to include label value, RecurrenceSegment to include recurrence pattern.
...
1. ~~Add focus trap to chat overlay (F-1)~~ — FIXED
2. ~~Add `aria-live="polite"` to Toast container (S-1)~~ — FIXED
3. ~~Add `role="alert"` to chat errors (E-2)~~ — FIXED
4. ~~Add `lang="en"` to `<html>` (X-1)~~ — FIXED
5. ~~Improve `muted-foreground` contrast values (C-1)~~ — FIXED
6. ~~Add ARIA menu roles to task/note action menus (A-3, A-4)~~ — FIXED
7. ~~Make hover actions keyboard-accessible with `group-focus-within:opacity-100` (K-1, K-2)~~ — FIXED
8. ~~Add tab semantics to Settings view (A-2)~~ — FIXED
9. ~~Add combobox pattern to SearchModal (A-6)~~ — FIXED
